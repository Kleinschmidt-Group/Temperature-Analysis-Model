"""
================================================================================
GCD Tailrace Water Temperature — Full Analysis & Figures (June–July–August)
================================================================================
Grand Coulee Dam (GCD) Tailrace — Historical Trend Analysis & Climate Projections
Primary Station of Interest: GCGW (Grand Coulee Dam Tailrace)
Supporting Stations: FDRW → CHJ → CHQW (provided for comparison and context)
...
WATER TEMPERATURE DATA:
  ...
  Stations: GCGW  — Grand Coulee Dam Tailrace  ← Primary station
            FDRW  — Grand Coulee Dam Forebay
            CHJ   — Chief Joseph Dam Forebay
            CHQW  — Chief Joseph Dam Tailrace (comparison)
  Format  : Hourly water temperature (°C and °F)
  Period  : GCGW: 2000–2025 (primary analysis)
            CHQW: 1997–2025 (comparison)
            Others (CHJ, FDRW): 2000–2025
...
--------------------------------------------------------------------------------
  ...
  Figures:
    ...
    Fig3_GCGW_Trend_Detail.png             — GCGW detailed trend + air overlay
...
# Label used to namespace the GitHub/Claude review bundle files (avoids Forebay/
# Tailrace collisions since both scripts share OUTPUT_DIR).
ANALYSIS_LABEL = 'Tailrace'
...
STATION_ORDER  = ['GCGW', 'FDRW', 'CHJ', 'CHQW']
STATION_LABELS = {
...
def load_station(name, path):
    ...
    df.attrs['qaqc'] = {
        ...
        'flags': {
            'Physical Bounds': int(bounds_count),
            'IQR Outliers (3.0xIQR)': int(outlier_count),
            'Constant Spans (24h+)': int(const_count),
            'Impossible Jumps (>3C/hr)': int(jump_count),
            'Data Gaps': int(gap_count),
        },
    }
    try:
        df.to_pickle(cache)
...
def load_air(path):
    ...
    # USBR AgriMet CSV. Metadata header block precedes the data; the data header
    # row starts with 'DATE'. Temperature columns are prefixed by the station id
    # (e.g. 'ABEI MN' or 'GCDW MN') with suffixes MN=min, MX=max, MM=mean, in °F.
    # Missing values coded as 'NO RECORD'. Auto-detect header row and columns so
    # the loader works regardless of station id or metadata length.
    with open(path, 'r', errors='ignore') as f:
        lines = f.readlines()
    hdr = next(i for i, l in enumerate(lines) if l.strip().upper().startswith('DATE'))
...
def run_analysis(stations, air):
    ...
    baseline_yr_label = f"{int(gcgw.year.min())}–{int(gcgw.year.max())}"
    climate_scenarios = [
    ...
    baseline_wt  = stations['GCGW'][stations['GCGW']['month'].isin(SUMMER_MONTHS)]['wtc'].mean()

    print(f"\n" + "=" * 76)
    print(f"  CLIMATE PROJECTIONS — GCGW (Grand Coulee Dam Tailrace)")
    print(f"=" * 76)
    ...
    for month, month_name in MONTH_TUPLES:
        ...
        gcgw_month      = stations['GCGW'][stations['GCGW']['month'] == month]['wtc']
        m_wt_baseline  = gcgw_month.mean()
        m_wt_max       = gcgw_month.max()
    ...
    for month, month_name in MONTH_TUPLES:
        ...
        water_month_daily = stations['GCGW'][stations['GCGW']['month'] == month]['wtc']
    ...
    # Week-by-week stats — GCGW JJA
    gcgw_summer = stations['GCGW'][stations['GCGW']['month'].isin(SUMMER_MONTHS)].copy()
    weekly_by_yr = gcgw_summer.groupby(['year', 'week', 'month'])['wtc'].mean().reset_index()
    ...
    print(f"\n  Week-by-week water temp stats (GCGW, Jun-Aug weeks):")
    ...
    results = dict(
        ...
        ann_gcgw=ann_gcgw,
        gcgw_month_trends=gcgw_month_trends,
...
def export_excel(results):
    ...
    ann_gcgw    = results['ann_gcgw']
    gcgw_month_trends = results['gcgw_month_trends']
    stations_data = results['stations']
    air_data     = results['air']

    gcgw_yr_range = f"{int(ann_gcgw.index.min())}–{int(ann_gcgw.index.max())}"
    ...
    GCGW_FILL = PatternFill(start_color='fde8e8', end_color='fde8e8', fill_type='solid')
    ...
    r = add_row(ws0, r, 'Stations', 'GCGW (GC Tailrace - PRIMARY), FDRW, CHJ, CHQW')
    ...
    # Sheet 5 — GCGW Annual (Jun, Jul, Aug, combined)
    ws3 = wb.create_sheet('GCGW Annual')
    ws3.append(['Year',
    ...
    style_header(ws3)
    jun_series = gcgw_month_trends['June']['ann']
    jul_series = gcgw_month_trends['July']['ann']
    aug_series = gcgw_month_trends['August']['ann']
    all_yrs = sorted(set(ann_gcgw.index) | set(jun_series.index) |
...
def make_figures(results):
    ...
    gcgw_yrs = stations['GCGW']['year']
    air_yrs  = air['year']

    def add_footnote(fig):
        pass

    all_years = sorted(stations['GCGW']['year'].unique())
    yr_cmap = cm.coolwarm
    ...
    gcgw_min_yr = stations['GCGW']['year'].min()
    gcgw_max_yr = stations['GCGW']['year'].max()
    gcgw_n_years = len(stations['GCGW'][stations['GCGW']['month'].isin(SUMMER_MONTHS)]
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # FIGURE 3 — GCGW detailed: Jun/Jul/Aug water + Jun/Jul/Aug Mohseni + air
    # ─────────────────────────────────────────────────────────────────────────
    print("  Figure 3 …")
    fig = plt.figure(figsize=(18, 52))
    gs  = gridspec.GridSpec(7, 1, height_ratios=[3, 3, 3, 3, 3, 3, 2.5], hspace=0.30)
    fig.suptitle(
        'Grand Coulee Dam Tailrace (GCGW) — June, July, August Temperature Trends\n'
    ...
    for month, month_name in MONTH_TUPLES:
        ...
        t = gcgw_month_trends[month_name]
        xm, ym = t['x'], t['y']
        slm, icm = t['sl'], t['ic']
        taum, pm = t['tau'], t['p']
    ...
    add_footnote(fig)
    plt.savefig(f'{OUTPUT_DIR}/Fig3_GCGW_Trend_Detail.png', dpi=150, bbox_inches='tight')
    plt.close()
    print("    ✓ Fig3_GCGW_Trend_Detail.png")
    ...
    print("  Figure 4 …")
    gcgw = stations['GCGW']
    summer = gcgw[gcgw['month'].isin(SUMMER_MONTHS)].copy()
    ...
    ax.plot(overall_daily.index, overall_daily.values,
            color='black', lw=3, alpha=0.9, zorder=5, label=f'{gcgw_n_years}-Year Average Daily Cycle')

    ax.set_xlabel('Day of Summer', fontsize=20)
    ax.set_ylabel('Water Temperature (°C)', fontsize=20)
    ax.set_title('Grand Coulee Dam Tailrace (GCGW) — Daily Mean Water Temperature\n'
                 f'June–August, Every Year {gcgw_min_yr}–{gcgw_max_yr}',
                 fontsize=20, fontweight='bold')
    ...
    ax.set_title('Mohseni Air–Water Temperature Regression — GCGW (GCD Tailrace)\n'
                 f'Weekly Averages, {gcgw_min_yr}–{gcgw_max_yr}', fontsize=20, fontweight='bold')
    ...
    ax.set_title('Mohseni Model — Observed vs. Predicted Weekly Water Temperature\n'
                 f'GCGW (GCD Tailrace), All Weeks {gcgw_min_yr}–{gcgw_max_yr}',
                 fontsize=20, fontweight='bold')
    ...
    fig.suptitle(
        ...
        f'GCGW (GCD Tailrace), {gcgw_min_yr}–{gcgw_max_yr}',
        fontsize=20, fontweight='bold'
    )
    ...
    for idx, (month, month_name) in enumerate(MONTH_TUPLES):
        ...
        ax.set_xlim(gcgw_min_yr - 1.5, gcgw_max_yr + 1.5)
    ...
    ax.set_ylabel('Weekly Mean Water Temperature (°C)', fontsize=20)
    ax.set_title(f'Mohseni Air–Water Temperature Relationship — All {gcgw_n_years} Years Overlaid\n'
                 f'GCGW (GCD Tailrace), Weekly Averages {gcgw_min_yr}–{gcgw_max_yr}',
                 fontsize=20, fontweight='bold')
    sm = cm.ScalarMappable(cmap=yr_cmap, norm=yr_norm)
    sm.set_array([])
    cbar = plt.colorbar(sm, ax=ax, pad=0.01, fraction=0.03, aspect=30)
    ...
    if monthly_proj:
        ...
        ax.set_title('Projected June, July, and August Water Temperature at GCD Tailrace (GCGW)\n'
    ...
    if monthly_proj:
        ...
        ax.set_title('Climate Projection Points on the Mohseni Air–Water Curve\n'
                     'GCGW (GCD Tailrace) — June (▲), July (○), August (□) — CMIP5 RCP 4.5 / 8.5',
                     fontsize=20, fontweight='bold')
    ...
    if monthly_proj:
        ...
        baseline_label_fig = f"{gcgw_min_yr}–{gcgw_max_yr}"
        ax.text(0.02, 0.97,
        ...
        ax.set_title('Projected Change in June, July, and August Water Temperature at GCD Tailrace (GCGW)\n'
                     'Mohseni (1998) Delta Method + CMIP5 Climate Scenarios (RCP 4.5 / 8.5)',
                     fontsize=20, fontweight='bold')
        ...
        ax.set_title('Mohseni Model Sensitivity Used for Monthly Climate Projections\n'
                     'GCGW (GCD Tailrace) — Delta Method (RCP 4.5 / 8.5)',
                     fontsize=20, fontweight='bold')
    ...
    if week_stats_df is not None and len(week_stats_df) > 0:
        ...
        ax.set_ylabel('Water Temperature (°C)', fontsize=20)
        ax.set_title(f'Observed Weekly Water Temperature at GCGW — June through August ({gcgw_min_yr}–{gcgw_max_yr})',
                     fontsize=20, fontweight='bold')
    ...
    if week_stats_df is not None and len(week_stats_df) > 0 and monthly_proj_fig:
        ...
        ax.set_xlabel('Week', fontsize=20)
        ax.set_ylabel('Water Temperature (°C)', fontsize=20)
        ax.set_title('Projected Weekly Water Temperature at GCGW — June through August\n'
    ...
    if warm_day_proj and monthly_proj:
        ...
        fig.suptitle('Projected Water Temperature at GCGW (GCD Tailrace)\n'
...
# ═════════════════════════════════════════════════════════════════════════════
# SECTION 5B — REVIEW BUNDLE (GitHub / Claude Code)
# ═════════════════════════════════════════════════════════════════════════════
...
def _jsonable(obj):
    """Recursively convert numpy/pandas objects into JSON-serializable values."""
...
def export_review_bundle(results, label, qaqc_meta, code_path):
    """Write a GitHub/Claude-friendly bundle summarizing data, methods, results."""
    print("\n  Exporting review bundle …")
    ts = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    stations = results['stations']
    air = results['air']
    ann_primary = results.get('ann_fdrw', results.get('ann_gcgw'))
    primary_name = 'FDRW' if 'ann_fdrw' in results else 'GCGW'
    pa, pj = results['popt_all'], results['popt_jja']
    pkeys = ['mu', 'alpha', 'gamma', 'beta']

    data_used = {}
    for name, df in stations.items():
        data_used[name] = {
            'label': STATION_LABELS.get(name, name),
            'n_records': int(len(df)),
            'year_min': int(df['year'].min()), 'year_max': int(df['year'].max()),
        }
    data_used['AIR'] = {
        'label': 'Air Temperature (AgriMet daily mean = (min+max)/2)',
        'n_records': int(len(air)),
        'year_min': int(air['year'].min()), 'year_max': int(air['year'].max()),
    }
    ...
    meta = {
        ...
        'code_snapshot': f'{label}_analysis_code_snapshot.py',
        'data_sources': {
            'water_stations': {n: STATION_FILES[n] for n in STATION_ORDER},
            'air_file': AIR_TEMP_FILE,
            'maca_tmax_file': MACA_TMAX_FILE,
            'maca_tmin_file': MACA_TMIN_FILE,
        },
        'data_used': data_used,
        'qaqc_flags': qaqc_meta,
        'mohseni_model': {
            'equation': 'Tw = mu + (alpha - mu) / (1 + exp(gamma * (beta - Ta)))',
            'params_all_weeks': {k: float(v) for k, v in zip(pkeys, pa)},
            'NSE_all_weeks': float(results['NSE_all']),
            'params_jja_only': {k: float(v) for k, v in zip(pkeys, pj)},
            'NSE_jja_only': float(results['NSE_jja']),
            'used_for_projections': 'all_weeks',
        },
        'trends': {
            'primary_water_jja': {
                'slope_C_per_yr': float(results['sl_c']), 'MK_tau': float(results['tau_c']),
                'MK_p': float(results['p_c']),
                'theil_sen_ci_lo': float(results['ci_lo_c']),
                'theil_sen_ci_hi': float(results['ci_hi_c']),
            },
            'air_jja': {
                'slope_C_per_yr': float(results['sl_a']), 'MK_tau': float(results['tau_a']),
                'MK_p': float(results['p_a']),
            },
        },
        'baseline': {
            'air_C': float(results['baseline_air']),
            'water_C': float(results['baseline_wt']),
        },
        'summary_by_station': _jsonable(results['summary_df']),
        'monthly_summary': _jsonable(results['monthly_summary_df']),
        'projections_jja': _jsonable(results['proj_df']),
        'monthly_projections': {m: _jsonable(df) for m, df in results['monthly_proj'].items()},
        'warm_day_projections': {m: _jsonable(df) for m, df in results['warm_day_proj'].items()},
        'weekly_water_stats': _jsonable(results['week_stats_df']),
        'figures': [{'file': f, 'description': d} for f, d in FIGURE_DESCRIPTIONS],
    }

    # ── JSON ──
    json_path = os.path.join(OUTPUT_DIR, f'{label}_results_summary.json')
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(_jsonable(meta), f, indent=2, default=str)
    print(f"    → Saved {json_path}")

    # ── Markdown ──
    m = meta
    L = []
    L.append(f"# {label} Water Temperature Analysis — Review Report\n")
    L.append(f"*Generated {ts}*\n")
    L.append(f"Primary station: **{primary_name}**  |  Summer months: {SUMMER_MONTHS}\n")
    L.append("This bundle summarizes the data, methods, and results of the analysis "
             "for review. The full source code is included alongside as "
             f"`{m['code_snapshot']}`.\n")

    L.append("## 1. Data Sources\n")
    L.append("**Water temperature (hourly, DART):**\n")
    for n, p in m['data_sources']['water_stations'].items():
        L.append(f"- `{n}` — {STATION_LABELS.get(n, n)}: `{p}`")
    L.append(f"\n**Air temperature (USBR AgriMet daily):** `{m['data_sources']['air_file']}`\n")
    L.append(f"**CMIP5 projection deltas (MACAv2-METDATA):**\n"
             f"- Tmax: `{m['data_sources']['maca_tmax_file']}`\n"
             f"- Tmin: `{m['data_sources']['maca_tmin_file']}`\n")

    L.append("## 2. Data Used (after QA/QC + date filter)\n")
    L.append("| Series | Label | Records | Start | End |")
    L.append("| --- | --- | --- | --- | --- |")
    for k, v in data_used.items():
        L.append(f"| {k} | {v['label']} | {v['n_records']:,} | {v['year_min']} | {v['year_max']} |")

    L.append("\n## 3. QA/QC Flag Counts\n")
    for series, info in qaqc_meta.items():
        flags = info.get('flags', {})
        L.append(f"**{series}** (source: `{info.get('source_file', 'n/a')}`)")
        L.append("| Check | Flagged |")
        L.append("| --- | --- |")
        for chk, cnt in flags.items():
            L.append(f"| {chk} | {cnt} |")
        L.append("")

    L.append("## 4. Methods\n")
    L.append("1. **Mann-Kendall** trend test on annual JJA means.")
    L.append("2. **Theil-Sen** slope estimator (95% CI).")
    L.append("3. **Mohseni (1998)** nonlinear logistic air-water regression:")
    L.append("   `Tw = mu + (alpha - mu) / (1 + exp(gamma * (beta - Ta)))`")
    L.append("4. **CMIP5 delta** projections (RCP 4.5 / 8.5, 2040s / 2080s).")
    L.append("5. **Warm-day** scenario at 90th-percentile (10% exceedance) air temp.\n")

    mm = m['mohseni_model']
    L.append("### Mohseni Parameters\n")
    L.append("| Model | mu | alpha | gamma | beta | NSE |")
    L.append("| --- | --- | --- | --- | --- | --- |")
    pa_ = mm['params_all_weeks']; pj_ = mm['params_jja_only']
    L.append(f"| All weeks | {pa_['mu']:.3f} | {pa_['alpha']:.3f} | {pa_['gamma']:.4f} | "
             f"{pa_['beta']:.3f} | {mm['NSE_all_weeks']:.4f} |")
    L.append(f"| JJA only | {pj_['mu']:.3f} | {pj_['alpha']:.3f} | {pj_['gamma']:.4f} | "
             f"{pj_['beta']:.3f} | {mm['NSE_jja_only']:.4f} |")
    L.append(f"\nProjections use the **{mm['used_for_projections']}** model.\n")

    tr = m['trends']
    L.append("### Trends (annual JJA)\n")
    L.append("| Series | Slope (°C/yr) | MK tau | MK p |")
    L.append("| --- | --- | --- | --- |")
    L.append(f"| Primary water ({primary_name}) | {tr['primary_water_jja']['slope_C_per_yr']:+.4f} | "
             f"{tr['primary_water_jja']['MK_tau']:.4f} | {tr['primary_water_jja']['MK_p']:.4f} |")
    L.append(f"| Air | {tr['air_jja']['slope_C_per_yr']:+.4f} | {tr['air_jja']['MK_tau']:.4f} | "
             f"{tr['air_jja']['MK_p']:.4f} |")

    L.append(f"\n**Observed baseline:** air {m['baseline']['air_C']:.2f} °C, "
             f"water {m['baseline']['water_C']:.2f} °C\n")

    L.append("## 5. Results\n")
    L.append("### Station Summary (JJA)\n")
    L.append(_df_to_md(results['summary_df']))
    L.append("\n### Monthly Summary\n")
    L.append(_df_to_md(results['monthly_summary_df']))
    L.append("\n### Combined JJA Projections\n")
    L.append(_df_to_md(results['proj_df']))
    for mon, df in results['monthly_proj'].items():
        L.append(f"\n### {mon} Projections\n")
        L.append(_df_to_md(df))
    for mon, df in results['warm_day_proj'].items():
        L.append(f"\n### {mon} Warm-Day (90th pct) Projections\n")
        L.append(_df_to_md(df))
    L.append("\n### Weekly Water Temperature Stats\n")
    L.append(_df_to_md(results['week_stats_df']))

    L.append("\n## 6. Figures\n")
    L.append("| File | Description |")
    L.append("| --- | --- |")
    for f, d in FIGURE_DESCRIPTIONS:
        L.append(f"| {f} | {d} |")

    md_path = os.path.join(OUTPUT_DIR, f'{label}_ANALYSIS_REPORT.md')
    ...
    # ── Code snapshot ──
    try:
        snap = os.path.join(OUTPUT_DIR, f'{label}_analysis_code_snapshot.py')
        shutil.copyfile(code_path, snap)
        print(f"    → Saved {snap}")
    except Exception as e:
        print(f"    [!] code snapshot skipped: {e}")
...
if __name__ == '__main__':
    print("=" * 70)
    print("GCD Tailrace Water Temperature — Full Analysis (Jun-Jul-Aug)")
    print("=" * 70)
    ...
    # Capture QA/QC metadata now — the date filter below drops DataFrame.attrs.
    qaqc_meta = {s: dict(stations[s].attrs.get('qaqc', {})) for s in stations}
    qaqc_meta['AIR'] = dict(air.attrs.get('qaqc', {}))
    ...
    print("\n[5] Exporting review bundle …")
    export_review_bundle(results, ANALYSIS_LABEL, qaqc_meta, __file__)
